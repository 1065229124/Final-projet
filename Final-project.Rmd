...
Our goal is to predict the  New York City Airbnb listing prices


and classification of a listing 

being above or below the median price using various machine learning models.In the end, I want to select the models with lowest MSE.
...
ggplot(bnb, mapping = aes(x = price)) +
        ...
        scale_x_log10()
```
# 3.1 Histogram and Frequency Distribution of neighbourhood areas 
```{r}
fd_location <- data.frame(cbind(Frequency = table(bnb$neighbourhood_group), Percent = prop.table(table(bnb$neighbourhood_group)) * 100))
fd_location <- freq_location[order(freq_location$Frequency),]
fd_location
```
...
85% of Airbnb listings are located in Brooklyn and Manhattan, suggesting there is more demand for these boroughs especially since Brooklyn is the most populous borough and Manhattan is the center of NYC
Most of the listings available in NYC are entire homes/apartments
The most expensive listings based on average price tend to be entire homes in Manhattan (average price of $249) and Brooklyn (average price of $178)
There does not seem to be a strong correlation between price and other variables such as minimum nights, number of reviews, reviews per month, and number of listings owned by hosts.
